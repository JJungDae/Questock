...
> 개정일: 2026-07-21  
> 개정 사유: 지원 종목 확정과 공동 기사 귀속·golden coverage 정합성 보완; P0 LLM stack을 LiteLLM + Gemini로 확정; 무료 Gemini 데이터 처리·리포트 외부 전송 정책 보완
...
- 확인 완료:
  ...
  - P0 LLM stack을 LiteLLM Python SDK + Gemini 단일 모델로 확정.
- 미확인 사항:
  - OpenDART API key, NAVER API HUB/Search credential, `GEMINI_API_KEY` 설정 여부.
...
- M1 목표:
  - 종목별 2건 이상.
  - `manifest_id`, `page`, `section`, `usage_note`, `external_llm_processing_allowed`, `file_hash`, `ingestion_version` 기록.
- 외부 LLM 처리:
  - `external_llm_processing_allowed=true`인 리포트만 Gemini 입력에 사용.
  - 허용 근거는 기존 `usage_note`에 기록.
  - 허용 여부가 불명확하거나 redaction이 필요한 자료는 fixed template·비LLM 경로만 사용.
- fallback:
...
- 가용성: credential·AI Studio quota·sanitized live smoke 전까지 미검증
- 무료 quota 실패 시 billing 자동 연결·유료 전환 금지
- 유료 모델 검토: MVP 완성 이후
- credential: `GEMINI_API_KEY`
- app config: `LLM_MODEL`
- LiteLLM Proxy·Router·자동 fallback·사용자 모델 선택은 P0 제외
- 유료 모델 비교·전환은 MVP 완성 전 제외
- Google Search Grounding은 자체 Evidence·citation 정책과 섞지 않기 위해 P0 제외
- 무료 등급에는 개인·기밀·민감정보와 외부 처리 미허용 리포트를 전송하지 않음
- 전체 세션·원문 전체 대신 질문과 선택 Evidence snippet만 최소 전송
- credential 제공 전에는 adapter mock·recorded response fixture만 사용
- live 호출 성공과 fixture 성공을 구분해 기록
...
- [x] LLM provider·호출 layer·기본 model 결정 — Gemini 2.5 Flash 무료 등급 + LiteLLM
- [ ] OpenDART/NAVER credential과 `GEMINI_API_KEY` 제공 방식 결정
...
- [ ] 리포트 사용 가능 목록과 OpenDART/NAVER/Gemini credential 방식 확인 후 M0 전체 확정
