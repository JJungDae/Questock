...
- 현재 동작:
  ...
  - main push 완료
  - 최초 M1-02 구현 커밋: `539e04cc797b18041470d0d82bbae334740d5b72`
  - M1-02 보완 커밋: `a727fccbf8175c53d5e62198ba47b1f7486df80f`
  - M1-02 보완 main push 완료: 예, 사용자가 검수 목적으로 push
  - resolver 구현 파일과 data fixture 존재.
- 미확인 사항:
...
7. `verification_status`가 `verified`인데 `corp_code`가 `None` 또는 빈 문자열이면 fixture validation 실패로 처리한다.
...
- corp_code:
  ...
  - `verification_status: verified`인데 `corp_code`가 `None`, `""`, `"   "`이면 fixture validation 실패
- ResolutionResult:
...
- [x] exact name/ticker/security_id resolved
- [x] alias resolved
- [x] NFKC 정규화와 빈 입력 not_found 테스트 통과
- [x] ambiguous 입력을 임의 첫 후보로 확정하지 않음
- [x] wrong ticker not_found
- [x] 우선주/해외 ticker unsupported
- [x] unsupported와 not_found 의미가 테스트로 분리됨
- [x] corp_code 검증 상태가 fixture에 명시됨
- [x] candidate/blocked corp_code는 반환 `SecurityIdentifier.corp_code`가 `None`
- [x] verified corp_code 누락 fixture validation 실패
- [x] canonical ID·ticker·alias 충돌과 빈 alias validation 실패
- [x] ResolutionResult 상태별 불변조건 테스트 통과
- [x] `matched_by` Literal 계약과 상태별 허용값 테스트 통과
- [x] 판정 우선순위 테스트 통과
- [x] provider 호출 없이 unit test만으로 동작 검증
- [x] M1-01 테스트 회귀 없음
...
## 18. CONDITIONAL PASS 보완 기록
- 기록 일시: 2026-07-21
- 이전 기준 SHA: `0358fd88e65b11082369d35cf37c8d3c3a3163ab`
- 최초 구현 SHA: `539e04cc797b18041470d0d82bbae334740d5b72`
- 보완 SHA: `a727fccbf8175c53d5e62198ba47b1f7486df80f`
- main push 완료: 예, 사용자가 검수 목적으로 push
- 최종 판정: PASS
- 보완 범위:
  - `_SecurityRecord`에서 `verification_status == "verified"`이고 `corp_code`가 `None` 또는 빈 문자열이면 `FixtureValidationError`
  - verified 상태의 corp_code 누락 fixture 실패 테스트 추가
  - 기존 candidate/blocked 반환 `SecurityIdentifier.corp_code is None` 동작 유지
- 실제 테스트 결과:
  - PYTHONPATH: `.test_deps;.`
  - 구현 환경 로컬 재실행: PASS
  - regression 명령: `python -m pytest tests/unit/test_core_models.py tests/unit/test_status_contracts.py tests/unit/test_security_resolver.py -q`
  - regression exit code: `0`
  - regression 출력: `73 passed in 0.14s`
  - import smoke 명령: `python -c "from app.core.resolver import SecurityResolver; print('ok')"`
  - import smoke exit code: `0`
  - import smoke 출력: `ok`
- 독립 검수 환경 재실행: `NOT_RUN`
- live OpenDART 검증: `NOT_RUN`, credential 및 live 검증 별도 승인 없음
- GitHub CI: `NOT_RUN`
- `.gitignore` 함께 변경한 이유:
  - 로컬 Python 3.14 환경에서 기존 `.deps` 접근이 불가해 검증용 dependency를 `.test_deps/`에 설치했고, 해당 로컬 산출물이 git 추적 대상에 들어가지 않도록 제외하기 위함.
- M1-01 Task Card를 함께 변경한 이유:
  - 사용자 승인에 따라 M1-01 완료 승인 상태 `PASS`, 완료 커밋 `0358fd88e65b11082369d35cf37c8d3c3a3163ab`, main push 완료 상태를 문서에 사실대로 반영하기 위함.
- 미실행:
  - GitHub CI 확인: NOT_RUN
  - M1-03/provider/retrieval/API/UI/LLM: NOT_RUN, 범위 제외
