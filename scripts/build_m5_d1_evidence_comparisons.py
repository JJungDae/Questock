...
_NEWS_COMPARISON_CLAIMS = {
    frozenset(
        ...
        "support_summary": (
            "증권사 리포트도 생산 차질·환율·인센티브·원재료 부담을 "
            "2분기 이익 감소의 배경으로 봤습니다. 특히 원재료 부담은 "
            "매일신문의 설명과 겹치므로, 한 기사에만 나온 해석이라기보다 "
            "당시 증권사 분석에서도 확인된 배경으로 볼 수 있습니다. "
            "다만 리포트는 당일 주가 하락의 직접 원인이라기보다 실적 "
            "부진을 이해하기 위한 보조 배경입니다. DART에서는 7월 23일 "
            "2분기 잠정실적 공시 제출이 확인됩니다. 이는 다음 날 기사와 "
            "주가 약세가 공식 발표 이후 나타난 반응임을 확인하는 시간 "
            "기준이지만, 현재 데이터는 공시 목록만 확인했으므로 구체적인 "
            "실적 수치나 부진 원인까지 대조한 것은 아닙니다."
        ),
    },
}
        ...
        output.append(
            {
                ...
                "topics": topics,
                "text": _disclosure_metadata_text(
...
def _disclosure_metadata_text(
    *,
    submitted_date: str,
    report_name: str,
) -> str:
    if "영업(잠정)실적" in report_name.replace(" ", ""):
        return (
            f"{submitted_date} DART에 '{report_name}' 공시가 제출됐습니다. "
            "이는 실적 관련 뉴스가 공식 발표 이후 나온 시장 반응인지 "
            "확인하는 시간 기준입니다. 다만 공시 목록 메타데이터만 "
            "확인했으므로 구체적인 실적 수치나 부진 원인까지 공시 "
            "본문과 대조한 것은 아닙니다."
        )
    return (
        f"{submitted_date} DART에 '{report_name}' 공시가 제출됐습니다. "
        "이 연결은 공시 목록 메타데이터를 공식 배경으로 사용하며, "
        "세부 본문 사실을 뜻하지 않습니다."
    )
